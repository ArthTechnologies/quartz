...
async function downloadPaperJars() {
    ...
    for (let i in paperVersions.versions) {
        ...
        const response = await fetch(`https://papermc.io/api/v2/projects/paper/versions/${version}/builds`);
...
async function downloadVelocityJars() {
    //use papermc api
    const response = await fetch("https://papermc.io/api/v2/projects/velocity");
    const velocityVersions = await response.json();
    for (let i in velocityVersions.versions) {
        let version = velocityVersions.versions[i];
        const response = await fetch(`https://papermc.io/api/v2/projects/velocity/versions/${version}/builds`);
        const builds = await response.json();
        const build = builds.builds[builds.builds.length - 1].build;
        let channel = "release";
        ...
        const link = `https://papermc.io/api/v2/projects/velocity/versions/${version}/builds/${build}/downloads/velocity-${version}-${build}.jar`;
